...
class VBoxProvider {
    ...
    async postSetup(ip, sshKeyPath, verbose) {
        ...
        let scpInterfaces = await execAsync(`scp -i ${sshKeyPath}  -P 2002 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -q -F /dev/null -o IdentitiesOnly=yes  /tmp/interfaces vagrant@127.0.0.1:/tmp/interfaces`);
        if(verbose)
            console.log(scpInterfaces.stdout);
        if(scpInterfaces.stderr)
            throw `failed to copy interfaces configuration`;

        // mv vm:/tmp/interfaces /etc/network/interfaces, 
        // sudo systemctl restart networking
        // ifdown ens0p8; ifup ens0p8
        let sshApplyInterfaces = await execAsync(`ssh -q -i ${sshKeyPath} -p 2002 -o StrictHostKeyChecking=no -F /dev/null -o IdentitiesOnly=yes vagrant@127.0.0.1 bash -c "sudo mv /tmp/interfaces /etc/network/interfaces && sudo systemctl restart networking; sudo ifdown enp0s8; sudo ifup enp0s8"`);
        if(verbose)
