...
class VBoxProvider {
    ...
    async micro(name, cpus, mem, iso, ssh_port, syncs, disk, verbose) {
        ...
        if( disk )
        {
            let diskPath = path.join(os.homedir(),'.baker','boxes',`${name}-disk.vmdk`);
            let diskSize = 32768;
            if( !fs.existsSync(diskPath) )
            {
                //let rawDisk = "\\\\.\\PhysicalDrive0";
                //await execute("internalcommands", `createrawvmdk -filename ${diskPath} -rawdisk "${rawDisk}"`, verbose);
                await execute("createmedium", `disk --format VDI --filename ${diskPath} --size ${diskSize} --variant Fixed`, verbose);
                //await execute("modifymedium", `${diskPath} --type writethrough`, verbose);
            }

            await execute("storagectl", `"${name}" --name "SCSI" --add sata`, verbose);
            await execute("storageattach", `${name} --storagectl "SCSI" --port 2 --device 0 --type hdd --medium "${diskPath}"`, verbose);
        }
        ...
        if( disk )
        {
            ...
            let cmd = `/sbin/mkfs.ext4 /dev/sda; mount /dev/sda /mnt/disk`;
