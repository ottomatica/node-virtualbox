...
class VBoxProvider {
     ...
     */
    async provision(name, ovf, iso, verbose) {
    ...
    async micro(name, iso, ssh_port, verbose) {
        await execute("createvm", `--name "${name}" --register`);
        await execute("modifyvm", `"${name}" --memory 512`);
        await execute("storagectl", `"${name}" --name IDE --add ide`);
        await execute("storageattach", `${name} --storagectl IDE --port 0 --device 0 --type dvddrive --medium "${iso}"`);
    ...
    async customize(name, ip, ssh_port, syncs, verbose) {
    ...
    async postSetup(vmname, ip, port, sshKeyPath, syncs, verbose) {
