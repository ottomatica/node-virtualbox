...
class VBoxProvider {
    ...
    async serialOut(name) {
      ...
      // Fix: ensure .baker exists
      if( !fs.existsSync(path.join(os.homedir(),'.baker')))
      {
        fs.mkdirSync(path.join(os.homedir(),'.baker'));
      }
    ...
    async micro(name, cpus, mem, iso, ssh_port, sshKeyPath, syncs, disk, verbose, quickBoot, bridged) {
        ...
        // NIC2 =======
        if (bridged) {
            let bridgedif = (await this.bridgedifs())[0].Name;
            await execute("modifyvm", `${name} --bridgeadapter2 "${bridgedif}"`, verbose);
            await execute("modifyvm", `${name} --nic2 bridged`, verbose);
            await execute("modifyvm", `${name} --nictype2 virtio`, verbose);
            // await execute("modifyvm", `${name} --macaddress2 auto`, verbose);
        }
    ...
    async customize(name, ip, ssh_port, forward_ports=[], syncs, verbose) {
        ...
        if( syncs.length > 0 )
        {
            ...
            for( var sync of syncs )
            {
                ...
                await execute(`setextradata`, `${name} VBoxInternal2/SharedFoldersEnableSymlinksCreate/${name}-0 1`, verbose);
                count++;
            }
        }
    ...
    async bridgedifs(){
        return new Promise(function (resolve, reject) {   
            exec(`${VBexe} list bridgedifs`, (error, stdout, stderr) => {
                if(error || stderr) {
                    console.error(`exec error: vboxmanage list`);
                    console.error(`=> ${error}, ${stderr}`);
                    reject(error);
