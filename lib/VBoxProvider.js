    ...
    async micro(name, cpus, mem, iso, ssh_port, syncs, disk, verbose) {
        ...
        if( disk )
        {
            ...
            if( !fs.existsSync(diskPath) )
            {
                ...
                await execute("createhd", `--format VMDK --filename ${diskPath} --size ${diskSize}`, verbose);
            }

            await execute("storagectl", `"${name}" --name "SATA" --add sata`, verbose);
            await execute("storageattach", `${name} --storagectl "SATA" --port 0 --device 0 --type hdd --medium "${diskPath}"`, verbose);
        }
