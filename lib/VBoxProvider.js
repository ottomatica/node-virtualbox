...
class VBoxProvider {
    ...
    async postSetup(ip, port, sshKeyPath, verbose) {
        ...
        let scpInterfaces = await execAsync(`scp -q -i ${sshKeyPath}  -P ${port} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -F /dev/null -o IdentitiesOnly=yes ${tmpFile} vagrant@127.0.0.1:/tmp/interfaces`);
        if(verbose)
