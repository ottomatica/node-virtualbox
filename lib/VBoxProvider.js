...
class VBoxProvider {
    ...
    async postSetup(ip, port, sshKeyPath, verbose) {
        ...
        let scpInterfaces = await execAsync(`scp -q -i ${sshKeyPath}  -P ${port} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -F /dev/null -o IdentitiesOnly=yes  /tmp/interfaces vagrant@127.0.0.1:/tmp/interfaces`);
        if(verbose)
        ...
        let sshApplyInterfaces = await execAsync(`ssh -q -i ${sshKeyPath} -p ${port} -o StrictHostKeyChecking=no -F /dev/null -o IdentitiesOnly=yes vagrant@127.0.0.1 'bash -c "sudo cp /tmp/interfaces /etc/network/interfaces && sudo systemctl restart networking && sudo ifdown enp0s8 && sudo ifup enp0s8"'`);
        if(verbose)
