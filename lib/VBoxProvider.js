...
class VBoxProvider {
    ...
    async postSetup(ip, sshKeyPath, verbose) {
        ...
        await fs.writeFile(path.join(os.tmpdir(), `/interfaces`), interfaces);
        
        // cp /tmp/interfaces vm:/tmp/interfaces
        let scpInterfaces = await execAsync(`scp -q -i ${sshKeyPath}  -P 2002 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -F /dev/null -o IdentitiesOnly=yes  ${path.join(os.tmpdir(), `/interfaces`)} vagrant@127.0.0.1:/tmp/interfaces`);
        if(verbose)
